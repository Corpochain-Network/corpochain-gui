...
describe('createDefaultValues', () => {
  ...
  describe('when an CCH wallet type is provided', () => {
    it('should return an object with an CCH entry', () => {
  ...
  describe('when the returned data is modified', () => {
    it('the original data is not modified', () => {
      ...
      values.offered.kop = [{ amount: '1' }];
      values.requested.kop = [{ amount: '2' }];
      const values2 = createDefaultValues();
      expect(values2.offered.kop).toStrictEqual([]);
    });
  });
});
