...
function FarmingRewardsHistoryCards() {
  ...
  const totalChiaFarmedCard = useMemo(() => {
    if (!data || isLoading) {
      return <CardSimple title={<Trans>Total CCH Farmed</Trans>} value="-" loading={isLoading} error={error} />;
    }
    ...
    return <CardSimple title={<Trans>Total CCH Farmed</Trans>} value={cardValue} loading={isLoading} error={error} />;
