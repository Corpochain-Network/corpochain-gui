...
test('Confirm that User cannot send a TCCH amount greater then in Wallet', async () => {
  ...
  console.log(`CCH Balance: ${balance}`);
  ...
  //await page.locator('text=Fee *TCCH >> input[type="text"]').fill('0.000005');
  ...
  await expect(page.getByText('Pending Balance0 TCCH')).toBeVisible();
});
