import { SyncingStatus, toBech32m } from '@serwisgalena/api';
import { useSpendCATMutation, useFarmBlockMutation } from '@serwisgalena/api-react';
import {
  AdvancedOptions,
  Button,
  EstimatedFee,
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(t`Cannot send corpochain to coloured address. Please enter a corpochain address.`);
    }

    if (address.includes('corpochain_addr') || address.includes('colour_desc')) {
    ...
    if (address.slice(0, 12) === 'corpochain_addr://') {
